...
collide_left=False
collide_right=False
xspeed_pl, yspeed_pl, stand, mouse_down, mid_x, mid_y, block_list, block_list_y, xofs,yofs,white, black = 0, 0, True, False, scw // 2, sch // 2, deque([]), deque([]), 0,0, (255, 255, 255), (0, 0, 0)
...
def block_render():
    global mouse_down, xofs, yofs,yspeed_pl, stand, collide_left,collide_right
    for h in range(len(block_list)):
        if abs(block_list[h][0][1] + xofs) < 550:
            for v in range(len(block_list[0])):
                ...
                block_rect.topleft = (block_list[h][v][1] + xofs + mid_x - (bls // 2), block_list[h][v][2]+yofs + bls)
                if block_list[h][v][0] != 0:
                    if block_list[h][v][0]==1:
                        screen.blit(dirt_block_surface,(block_list[h][v][1]+xofs+mid_x-(bls//2),block_list[h][v][2]+yofs+bls))
                    elif block_list[h][v][0]==2:
                        screen.blit(stone_block_surface,(block_list[h][v][1]+xofs+mid_x-(bls//2),block_list[h][v][2]+yofs+bls))
...
def player_render():
    global stand, yspeed_pl, xofs,yofs,collide_left,collide_right
    ...
    if keys[pg.K_d]:
        ...
        if collide_right:
            ...
            collide_right=False
        collide_right=False
        player_rect.center=(mid_x,mid_y)
